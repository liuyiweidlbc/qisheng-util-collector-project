...
(function () {
    ...
    const MATCH_NAV_STICK_MS = 45000;
    ...
    /** 第二层：脚本本地防重（recHash / 进行中），需开关开启且第一层已通过 */
    ...
    function isBetAttemptBlocked(recHash) {
        if (!isBetDedupEnabled() || !recHash) return false;
        return !!getBetAttempt(recHash);
    }

    function markBetAttempt(recHash, option, stake) {
        if (!isBetDedupEnabled() || !recHash) return;
        const store = pruneBetAttemptStore(loadBetAttemptStore());
        store[recHash] = {
            at: Date.now(),
    ...
    function isUncertainBetFailure(err) {
        ...
        return /超时|响应|订单|确认|网络/i.test(msg);
    }
    ...
    function clickSidebarFootball() {
        ...
        if (href && href.indexOf('/sportEvents/inplay/football') < 0 &&
            href.indexOf('/sportEvents/football/inplay') < 0) {
            console.warn('[hty-inplay] 保活：足球入口非滚球列表，改直链', href);
    ...
    function clickMatchOnInplayList(targetId, home, away) {
        ...
        const links = document.querySelectorAll(
            'a[href*="/sportEvents/inplay/football/match/"], a[href*="/sportEvents/incoming/football/match/"], a[href*="/match/"]'
        );
        for (let i = 0; i < links.length; i++) {
    ...
    function finishListPageEnterMatch(fallbackId, reason) {
        ...
        if (fallbackId) gotoInplayMatchDirect(fallbackId, null, reason || '列表页直跳比赛');
    }
    ...
    function bootListPageEnterMatch() {
        ...
        async function tryClick() {
            ...
            if (listPageEnterAttempts > maxAttempts) {
                ...
                const fallbackId = isMatchLocallyEnded(targetId)
                    ? pickNextNavigableMatchId([targetId])
                    : targetId;
                finishListPageEnterMatch(fallbackId, '列表页超时直跳');
                return;
            }
            const ready = countInplayListMatchLinks() > 0;
            if (!ready) {
    ...
    function bootListPageKeepAlive() {
        ...
        async function listPageNavigateToKickoffMatch() {
            ...
            try {
                ...
                gotoInplayMatchDirect(targetId, null, '列表页进入策略比赛');
            } catch (e) {
                console.warn('[hty-inplay] 列表页检测开赛失败', e);
            }
        }

        setInterval(listPageNavigateToKickoffMatch, INPLAY_MATCH_WATCH_MS);
        if (document.body) listPageNavigateToKickoffMatch();
        else document.addEventListener('DOMContentLoaded', listPageNavigateToKickoffMatch);
    }
    ...
    function clickMarketCategoryTab(tab) {
        ...
        for (let i = 0; i < nodes.length; i++) {
            ...
            if (tid && (tid === tabId || tid.indexOf(tabId) >= 0)) {
                safeClick(el);
                return true;
            }
            const text = (el.textContent || '').replace(/\s+/g, '');
    ...
    function gotoInplayMatch(id, tab, forceRefresh) {
        ...
        recordInplayNavigation(id);
        window.location.href = url;
        return true;
    }

    function gotoInplayMatchDirect(id, tab, reason) {
        if (!id) return false;
        if (shouldBlockAutoMatchNavigation(id, reason)) {
        ...
        console.log('[hty-inplay] 直接跳转滚球页', id, tab || 'all', reason || '');
        recordInplayNavigation(id);
        window.location.href = inplayMatchUrl(id, tab);
        return true;
    }
    ...
    function openInplayMatchPage(targetId, reason) {
        ...
        recordInplayNavigation(targetId);
    ...
    function pickPreferredNavigableMatch(excludeId, currentId, sourceList) {
        ...
        const recentKickoff = navigable.filter(function (item) {
            ...
            return parseKickoffMs(b.kickoffTime) - parseKickoffMs(a.kickoffTime);
        });
    ...
    function pickRotatingInplayMatch(currentId, sourceList) {
        ...
        if (cur) {
            ...
            try {
                const saved = parseInt(sessionStorage.getItem(KEEPALIVE_ROTATE_INDEX_KEY) || '0', 10);
        ...
        saveRotateIndex(pickIdx + 1, ids.length);
        return picked;
    }
    ...
    function pickInplayNavigableMatch(excludeId, sourceList) {
        ...
        return pickPreferredNavigableMatch('', matchId, sourceList);
    }
    ...
    async function fillSimplePassword(pin) {
        ...
        for (let i = 0; i < 4; i++) {
            const btn = findDigitButtonIn(searchRoot, digits.charAt(i));
            if (!btn) break;
            safeClick(btn);
    ...
    async function tryAutoRelogin(options) {
        ...
        if (Date.now() - lastReloginAttemptAt < cooldown) return false;
    ...
    function scheduleLoginWatch() {
        ...
        function tick() {
            ...
            loginWatchTimer = setTimeout(tick, loggedIn ? SESSION_LOGIN_WATCH_MS : RELOGIN_WATCH_MS);
            if (isIdleLoginModalVisible()) {
                dismissIdleLoginModal();
                return;
            }
            ...
            if (!nowLoggedIn && !reloginInProgress) {
                const urgent = placing || wasLoggedIn;
    ...
    async function runKeepaliveSwitch() {
        ...
        if (isLoggedIn() &&
            htyApiState.apiBase &&
            htyApiState.headers.authorization &&
            Date.now() - htyApiState.lastCaptureAt <= SESSION_HEADER_FRESH_MS) {
            try {
                await sessionApiKeepAlive();
                return;
            } catch (e) {
                console.warn('[hty-inplay] API保活失败，尝试页面保活', e);
            }
        }

        if (Date.now() - lastPageKeepaliveAt < PAGE_KEEPALIVE_MIN_GAP_MS) return;

        const targetId = resolveKeepaliveTargetId();
        if (!targetId || !isMatchIdInPlay(targetId)) {
            console.log('[hty-inplay] 保活：无进行中比赛，跳过页面跳转');
            return;
        }
        lastPageKeepaliveAt = Date.now();
    ...
    function rememberExecutedStrategy(recHash, orderno, option) {
        ...
        // 仅标记实际下注的那条策略；testid 仅用于脚本防重复下单，不联动其他阈值
    ...
    const BET_SUBMITTED_CONFIRM_LABELS = ['确定', 'OK', '确认'];
    ...
    async function dismissBetSubmittedDrawer(maxRounds) {
        const rounds = maxRounds != null ? maxRounds : 8;
        for (let i = 0; i < rounds; i++) {
            if (!isBetSubmittedDrawerVisible()) return true;
            ...
            if (btn) {
                ...
                await humanDelay(280, 480);
            }
        }
        if (isBetSubmittedDrawerVisible()) {
            console.warn('[hty-inplay] 已提交抽屉仍在，未能自动点击确定');
        }
        return !isBetSubmittedDrawerVisible();
    }
    ...
    async function waitForBetOutcomeAfterSubmit(betWait, option, sinceAt) {
        const deadline = Date.now() + BET_API_WAIT_MS + 8000;
        let apiPayload = null;
        let apiError = null;
        let apiSettled = false;

        betWait.then(function (p) {
            apiPayload = p;
            apiSettled = true;
        }).catch(function (e) {
            apiError = e;
        ...
        while (Date.now() < deadline) {
            if (apiPayload) {
                return { source: 'api', payload: apiPayload };
            }
            if (isBetSubmittedDrawerVisible()) {
                setBetStep('检测到投注成功抽屉，点击确定…');
                renderPanel(true);
                ...
                if (captured) {
                    return { source: 'api_late', payload: captured };
                }
                ...
                if (recovered) {
                    ...
                    if (orderno.indexOf('ui-') === 0) {
                        return { source: 'ui', record: recovered };
                    }
                    return { source: 'order', record: recovered };
                }
                console.log('[hty-inplay] 已提交抽屉确认，按 UI 成功处理');
                return { source: 'ui', record: buildBetRecordFromUiSuccess(option) };
            }
        ...
        if (apiPayload) return { source: 'api', payload: apiPayload };
        if (isBetSubmittedDrawerVisible()) {
            await dismissBetSubmittedDrawer();
            const captured = consumeLastCapturedBetSuccess(option, sinceAt);
            if (captured) return { source: 'api_late', payload: captured };
            const recovered = await tryRecoverSuccessfulBet(option, sinceAt, {
                quick: true,
            });
            if (recovered) {
        ...
        if (lateCaptured) return { source: 'api_late', payload: lateCaptured };
    ...
    async function tryRecoverSuccessfulBet(option, sinceAt, retryOpts) {
        ...
        if (canQueryOrdersReport() || getCachedOrdersReportText(matchId)) {
            const retries = opts.quick === false && opts.retries != null
                ? opts.retries
                : 1;
    ...
    async function finalizeBetSuccess(option, betRecord, fromAutoBet, extraMsg) {
        ...
        rememberExecutedStrategy(recHash, betRecord.orderno, option);
        ...
        if (recHash) {
            ...
            try {
                ...
                console.warn('[hty-inplay] 策略状态接口失败，已本地标记执行', recHash, msg);
            }
        }
        clearBetInFlight();
    ...
    /** API 为准：不覆盖 ruleMeetIgnore；服务端已改回未执行时清除本地陈旧缓存 */
        ...
        for (let i = 0; i < strategyList.length; i++) {
            ...
            if (apiIgnore === '2') {
                if (!store[item.recHash]) {
                    store[item.recHash] = { orderno: '', at: Date.now() };
                    dirty = true;
                }
                continue;
            }
    ...
    function shouldAutoBet() {
        ...
        if (isBetDedupEnabled() && recHash && isBetAttemptBlocked(recHash)) return false;
        return true;
    }
    ...
    async function placeTestBet(option, fromAutoBet) {
        ...
        if (isBetDedupEnabled() && recHash && isBetAttemptBlocked(recHash)) {
            const attempt = getBetAttempt(recHash);
            let recovered = null;
            if (isBetSubmittedDrawerVisible()) {
        ...
        const inflight = getBetInFlight();
        if (isBetDedupEnabled() && recHash && inflight && inflight.recHash === recHash) {
        ...
        try {
            ...
            let betWait = armBetResultWaiter();
            function rearmBetWaiter() {
                clearBetResultWaiter();
                betWait = armBetResultWaiter();
            }
            await ensureBetCartVisible();
            await submitBetSlip(stakeInput, rearmBetWaiter);
            const outcome = await waitForBetOutcomeAfterSubmit(betWait, option, betAttemptAt);
            let betRecord;
            let outcomeHint = '';
            ...
            const keepAttempt = betAttemptAt > 0 && isUncertainBetFailure(err);
            if (keepAttempt) {
                if (isBetDedupEnabled()) {
                    markBetAttempt(recHash, option, stakeInput);
                    markBetInFlight(recHash, {
