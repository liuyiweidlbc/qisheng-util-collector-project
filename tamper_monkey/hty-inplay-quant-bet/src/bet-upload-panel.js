/** Bet-record upload panel, bundled from 8868_betrecord.js into the HTY quant script. */
export function bootBetUploadPanel() {
  if (typeof window !== 'undefined' && window.__tmHtyBetUploadBooted) return;
  try { window.__tmHtyBetUploadBooted = true; } catch (e) { /* ignore */ }
      ...
      function isUploadCaptureUrl(url) {
          var text = String(url || '');
          return text.includes('platform/thirdparty-report/user/orders/sport?betStatus=') ||
              text.includes('socbet') ||
              text.includes('platform/payment/wallets/list') ||
              text.includes('/product/cashout/setting');
      }
...
export function syncBetUploadPanelToRoute() {
  try {
    window.dispatchEvent(new CustomEvent('tm-hty-quant-route', {
      detail: { history: /\/history(\/|$)/i.test(String(window.location.pathname || '')) }
    }));
  } catch (e) { /* ignore */ }
}
